...
class sprite_block(pygame.sprite.Sprite):
    ...
    def __init__(self, sprite_list=[], start_block_x=TILE_SIZE, start_block_y=TILE_SIZE, sprite_type=None):
        ...
        self.sprite_sheet = sprite_assets(sprite_type, [64, 64, 64, 64, 64, 64, 64, 64, 64, 64, 64, 64, 64, 80, 128, 144, 160])
        self.sprite_asset = self.sprite_sheet.get_sprite_image()
        self._layer = GROUND_LAYER
        self.groups = self.get_sprite_list().blocks
        pygame.sprite.Sprite.__init__(self, self.groups)

        self.image = self.sprite_asset.get_sprite(self.sprite_sheet.sprite_x(), self.sprite_sheet.sprite_y(),
                                                  self.get_block_width(), self.get_block_height())
